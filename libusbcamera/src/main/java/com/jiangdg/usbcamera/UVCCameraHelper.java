...
public class UVCCameraHelper {
    ...
    public interface OnMyDevConnectListener {
        ...
        void onConnectDev(UsbDevice device, boolean isConnected, SurfaceTexture[] outputs);

        void onDisConnectDev(UsbDevice device);
    }

    public SurfaceTexture[] outputs;

    public void initUSBMonitor(Context activity, View cameraView, final OnMyDevConnectListener listener, SurfaceTexture[] outputs) {
        this.mActivity = activity;
        this.mCamView = cameraView;
        this.outputs = outputs;

        Toast.makeText(mActivity, "initUSBMonitor", Toast.LENGTH_SHORT).show();
...
//                Toast.makeText(mActivity, "onAttach", Toast.LENGTH_SHORT).show();
                if (listener != null) {
...
//                Toast.makeText(mActivity, "onDettach", Toast.LENGTH_SHORT).show();
                if (listener != null) {
...
//                Toast.makeText(mActivity, "onConnect", Toast.LENGTH_SHORT).show();
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        // start previewing
                        startPreview(outputs);
                    }
                }).start();
                if(listener != null) {
                    listener.onConnectDev(device,true, outputs);
                }
            }
...
//                Toast.makeText(mActivity, "onDisconnect", Toast.LENGTH_SHORT).show();
                if (listener != null) {
...
//        mCamView.setAspectRatio(previewWidth / (float)previewHeight);
        mCameraHandler = UVCCameraHandlerMultiSurface.createHandler(mActivity, mCamView, 2,
...
//            throw new NullPointerException("mUSBMonitor ="+mUSBMonitor+"deviceFilters=;"+deviceFilters);
    ...
    public void startPreview(SurfaceTexture[] sts) {
        if (mCameraHandler != null) {
            for (SurfaceTexture st:sts) {
